...
// docs/PRD-video-prompt-coach.md بخش ۴.۲ — مودال چت «بررسی پرامپت». الگوی ظاهری از
// PromptLibraryModal.tsx (overlay + کارت گرد slate-950)، رنگ تاکیدی بنفش (نه سبز برند) طبق
// دیزاین. تاریخچه فقط در state این کامپوننت زندگی می‌کند — با بسته‌شدن مودال از بین می‌رود
// (بخش ۱: هیچ رکورد جدیدی در دیتابیس ساخته نمی‌شود).
...
export function PromptReviewModal({
  ...
  initialReferenceAssets: PromptReviewReferenceAsset[]
  onApplyPrompt: (prompt: string) => void
}) {
  const [messages, setMessages] = useState<ChatMsg[]>([])
  const [draft, setDraft] = useState('')
  const review = useVideoPromptReview()
  const scrollRef = useRef<HTMLDivElement>(null)
  const wasOpen = useRef(false)

  function send(history: ChatMsg[], referenceAssets?: PromptReviewReferenceAsset[]) {
    ...
    review.mutate(
      { referenceAssets, messages: payloadMessages },
      {
        onSuccess: res => {
  ...
  // initialReferenceAssets شروع کن — بخش ۴.۲: بستن و بازکردن دوباره یعنی تاریخچه پاک شود
  useEffect(() => {
    if (open && !wasOpen.current) {
      const first: ChatMsg = { role: 'user', text: initialPrompt }
      setMessages([first])
      setDraft('')
      send([first], initialReferenceAssets)
    }
