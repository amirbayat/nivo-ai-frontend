...
function priorityBadge(priority: Priority) {
  const map: Record<Priority, { label: string; cls: string }> = {
    LOW: { label: fa.ticket.priority.LOW, cls: 'text-slate-400 light:text-slate-500' },
    NORMAL: { label: fa.ticket.priority.NORMAL, cls: 'text-slate-300 light:text-slate-600' },
    HIGH: { label: fa.ticket.priority.HIGH, cls: 'text-amber-400 light:text-amber-700' },
    URGENT: { label: fa.ticket.priority.URGENT, cls: 'text-red-400 light:text-red-700' },
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-slate-100 light:text-slate-900">{fa.settings.tickets}</h2>
        <button
          onClick={() => {
      ...
      {submitted && (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-400 light:text-emerald-700">
          {fa.ticket.created}
        </div>
      )}
      ...
      {showForm && (
        <div className="rounded-2xl border border-slate-700/60 bg-slate-800/40 p-5 light:border-slate-200 light:bg-slate-50">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-1.5 block text-sm text-slate-400 light:text-slate-600">{fa.ticket.subject}</label>
              <input
                type="text"
                value={subject}
                onChange={e => setSubject(e.target.value)}
                required
                className="w-full rounded-xl border border-slate-700 bg-slate-800/80 px-4 py-3 text-sm text-slate-200 placeholder-slate-500 outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30 light:border-slate-300 light:bg-white light:text-slate-900 light:placeholder:text-slate-400 transition-colors"
                placeholder={fa.ticket.subject}
              />
            </div>
            <div>
              <label className="mb-1.5 block text-sm text-slate-400 light:text-slate-600">{fa.ticket.body}</label>
              <textarea
                value={body}
                onChange={e => setBody(e.target.value)}
                required
                rows={4}
                className="w-full resize-none rounded-xl border border-slate-700 bg-slate-800/80 px-4 py-3 text-sm text-slate-200 placeholder-slate-500 outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30 light:border-slate-300 light:bg-white light:text-slate-900 light:placeholder:text-slate-400 transition-colors"
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="text-sm text-slate-400 hover:text-slate-200 light:text-slate-500 light:hover:text-slate-800 transition-colors"
              >
                {fa.common.cancel}
              </button>
            ...
            {tickets.map(ticket => (
              ...
              >
                <div className="flex flex-col gap-1 min-w-0">
                  <span className="text-sm font-medium text-slate-200 light:text-slate-900 truncate">{ticket.subject}</span>
